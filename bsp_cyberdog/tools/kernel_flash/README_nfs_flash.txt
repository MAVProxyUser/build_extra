...
Once you have access to the device terminal, You need to disable automount on
your device temporarily. On most distributions of Debian-based Linux, you can do this
using the following command:
      $ systemctl stop udisks2.service

Then, you can run the following command on the target's NFS to flash target device:
...
Case 1. Run commands below to flash external storage device alongside with internal storage:
...
Then, you can run the following command on the target terminal to flash external storage device only.
...
Case 2. Run commands to flash to external device only and rcm-boot to NFS:
